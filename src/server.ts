...
db.serialize(async () => {
  ...
  const usersCount = (
    await query<CountResult>("SELECT count(*) as c FROM users")
  )[0].c;
  if (usersCount === 0) {
    console.log("🌱 Seeding Users...");
    run(
      "INSERT INTO users (login, password, name, role, balance) VALUES (?,?,?,?,?)",
      ["admin", md5("admin"), "Администратор", "ADMIN", 0],
    );
    run(
      "INSERT INTO users (login, password, name, role, balance) VALUES (?,?,?,?,?)",
      ["cook", md5("cook"), "Шеф Повар", "COOK", 0],
    );
  }

  const prodCount = (
    await query<CountResult>("SELECT count(*) as c FROM products")
  )[0].c;
  if (prodCount === 0) {
    ...
    const products = [
      {
        ...
        stock: 15,
        image: "🍗",
        desc: "С курицей и маслом",
      },
      {
        id: 2,
        category: "lunch",
        name: "Пюре с подливой",
        price: 80,
        calories: 150,
        stock: 50,
        image: "🥔",
        desc: "Домашнее на молоке",
      },
      {
        id: 3,
        category: "lunch",
      ...
      },
      {
        ...
        stock: 30,
        image: "🍫",
        desc: "Молочный с орехом",
      },
    ];
    products.forEach((p) => {
      run(
        "INSERT INTO products (id, category, name, price, calories, stock, image, description) VALUES (?,?,?,?,?,?,?,?)",
        [
          p.id,
          p.category,
          p.name,
...
app.get("/api/init", async (req: Request, res: Response) => {
  try {
    ...
    const sanitary = (
      await query<Setting>(
        "SELECT value FROM settings WHERE key='sanitary_day'",
      )
    )[0];
    res.json({
      products,
...
app.post("/api/auth/register", async (req: Request, res: Response) => {
  try {
    ...
    await run(
      "INSERT INTO users (login, password, name, role, balance) VALUES (?,?,?,?,?)",
      [login, md5(password), name, "STUDENT", 0],
    );
...
app.post("/api/orders", async (req: Request, res: Response) => {
  try {
    ...
    if (sanitary && sanitary.value === "true") {
      return res.json({
        success: false,
        error: "Столовая закрыта (Санитарный день)",
      });
    }
    ...
    if (!payWithSub && user.balance < total)
      return res.json({ success: false, error: "Мало средств" });
    ...
    const result = await run(
      "INSERT INTO orders (user_id, user_name, items, total, status, date) VALUES (?, ?, ?, ?, 'pending', ?)",
      [
        userId,
        user.name,
...
app.post("/api/orders/confirm", async (req: Request, res: Response) => {
  try {
    ...
    await run(
      "UPDATE orders SET status = 'completed', rating = ? WHERE id = ?",
      [rating, orderId],
    );
...
app.post("/api/user/subscribe", async (req: Request, res: Response) => {
  ...
  await run(
    "UPDATE users SET balance = balance - 3000, subscription_end = ? WHERE id=?",
    [date.toLocaleDateString("ru-RU"), req.body.userId],
  );
...
app.post("/api/supplies/request", async (req: Request, res: Response) => {
  await run(
    "INSERT INTO supplies (product_name, amount, status, date) VALUES (?, ?, 'pending', ?)",
...
app.get("/api/admin/dashboard", async (req: Request, res: Response) => {
  try {
    const revenue =
      (
        await query<SumResult>(
          "SELECT SUM(total) as t FROM orders WHERE status != 'pending'",
        )
      )[0].t || 0;

    const orders = (
      await query<Order>("SELECT * FROM orders ORDER BY id DESC LIMIT 50")
    ).map((o) => ({ ...o, items: JSON.parse(o.items) }));

    const supplies = await query<Supply>(
      "SELECT * FROM supplies ORDER BY id DESC",
    );

    const allOrders = (
      await query<Order>(
        "SELECT items FROM orders WHERE status='completed' LIMIT 200",
      )
    ).map((o) => JSON.parse(o.items) as OrderItem[]);
    ...
    const ratingRow = (
      await query<AvgResult>(
        "SELECT AVG(rating) as r FROM orders WHERE rating > 0",
      )
    )[0];
...
app.post("/api/admin/supplies/approve", async (req: Request, res: Response) => {
  const sup = (
    await query<Supply>("SELECT * FROM supplies WHERE id=?", [req.body.id])
  )[0];
  if (sup) {
    await run("UPDATE products SET stock = stock + 50 WHERE name LIKE ?", [
      `%${sup.product_name}%`,
    ]);
    await run("UPDATE supplies SET status = 'approved' WHERE id=?", [
      req.body.id,
    ]);
    res.json({ success: true });
  }
